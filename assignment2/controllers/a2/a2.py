...
def run_robot(robot, help, goal):
    ...
    epuck_p1 = robot.getFromDef('e_puck_p1')
    epuck_ev = robot.getFromDef('e_puck_p2')
    ...
    obstacle_seg = []#[((0, -0.5),(1, -0.5)), ((-1, 0),(0.5, 0)), ((-0.29, 0.5),(1, 0.5))]
  
    
    while robot.step(time_step) != -1:
        pos_evader = epuck_ev.getPosition()[:2]
        pos_p1 = epuck_p1.getPosition()[:2]
        ...
        # found or not! 
        not_found_p1 = [True for _ in range(len(obstacle_seg))]
        not_found_p2 = [True for _ in range(len(obstacle_seg))]
        for idx, obs_seg in enumerate(obstacle_seg):
            not_found_p1[idx] = bool(help.check_intersect((pos, pos_evader), obs_seg))
            not_found_p2[idx] = bool(help.check_intersect((pos_p1, pos_evader), obs_seg))
            
        # if not any(not_found_p1) or not any(not_found_p2):
